...
알고리즘 자체는 9점대의 정교한 랭킹 손실 구현이지만 tf.compat.v1·Estimator 결합, 정적 shape 검증 의존, 하드코딩된 수치 마스킹과 방어 계층 부족이 운영 안정성을 깎고 있으므로 핵심 랭킹 수학은 보존한 채 입력 계약·수치 안정성·예외 방어를 강화하고 레거시 실행 계층을 점진적으로 격리하는 방향이 가장 안전한 고도화 전략이다.

제안패치
# Copyright 2019 The TensorFlow Ranking Authors.
#
# Modified for production hardening and defensive validation.
...
def _validate_tensor_not_empty(tensor, name="Tensor"):
    """Validates rank and prevents statically/dynamically empty tensors."""
    tensor = tf.convert_to_tensor(value=tensor)

    shape = tensor.get_shape()
    if shape.ndims is not None:
        if shape.ndims == 0:
            raise ValueError(f"{name} must have rank >= 1, got scalar tensor.")

        for dim in shape.as_list():
            if dim == 0:
                raise ValueError(
                    f"{name} contains a zero-sized dimension: {shape}"
                )

    # Static validation cannot detect dimensions represented by None.
    # Add runtime assertions for graph execution.
    runtime_assertions = [
...
def _check_tensor_shapes(tensors):
    """Checks rank, non-empty state, and compatible tensor shapes."""
    if not tensors:
        raise ValueError("Tensor list cannot be empty for shape checking.")

    checked = []

    for idx, tensor in enumerate(tensors):
        tensor_name = f"Tensor[{idx}]"
        tensor = _validate_tensor_not_empty(tensor, tensor_name)

        # Preserve the original static contract.
        tensor.get_shape().assert_has_rank(2)
        checked.append(tensor)

    base_shape = checked[0].get_shape()

    for idx, tensor in enumerate(checked[1:], start=1):
        tensor.get_shape().assert_is_compatible_with(base_shape)

    return checked


def _apply_pairwise_op(op, tensor):
    """Applies an operation pairwise over a rank-2 tensor."""
    tensor = _check_tensor_shapes([tensor])[0]
...
def _get_valid_pairs_and_clean_labels(labels):
    """Returns valid-pair mask and labels with invalid entries zeroed."""
    labels = _check_tensor_shapes([labels])[0]

    is_valid = utils.is_label_valid(labels)

    valid_pairs = _apply_pairwise_op(
        tf.logical_and,
        is_valid,
    )

    cleaned_labels = tf.compat.v1.where(
        is_valid,
        labels,
        tf.zeros_like(labels),
    )

    return valid_pairs, cleaned_labels
...
class _LambdaWeight(object):
    ...
    @abc.abstractmethod
    def pair_weights(self, labels, ranks):
        """Returns pairwise lambda weights."""
        raise NotImplementedError("Calling an abstract method.")
...
class DCGLambdaWeight(_LambdaWeight):
    """LambdaWeight for Discounted Cumulative Gain."""
    ...
    def __init__(
        ...
        if topn is not None and topn <= 0:
            raise ValueError(
                f"topn must be positive when specified, got {topn}"
            )

        if not 0.0 <= smooth_fraction <= 1.0:
            raise ValueError(
                "smooth_fraction must be in range [0, 1], "
                f"got {smooth_fraction}"
            )

        if not callable(gain_fn):
            raise TypeError("gain_fn must be callable.")

        if not callable(rank_discount_fn):
            raise TypeError("rank_discount_fn must be callable.")
    ...
    def pair_weights(self, labels, ranks):
        """Returns pairwise DCG lambda weights."""
        with tf.compat.v1.name_scope("dcg_lambda_weight"):
            _check_tensor_shapes([labels, ranks])
            ...
            pair_gain = _apply_pairwise_op(tf.subtract, gain)
            pair_gain *= tf.cast(valid_pair, tf.float32)
            ...
            relative_discount = tf.compat.v1.where(
                tf.greater(rank_diff, 0.0),
                tf.abs(
                    self._rank_discount_fn(rank_diff)
                    - self._rank_discount_fn(rank_diff + 1.0)
                ),
                tf.zeros_like(rank_diff),
            )
            ...
            pair_discount = (
                (1.0 - self._smooth_fraction) * relative_discount
                + self._smooth_fraction * absolute_discount
            )
    ...
    def individual_weights(self, labels, ranks):
        """Returns individual DCG weights."""
        _check_tensor_shapes([labels, ranks])
...
def _validate_finite(tensor, name):
    """Adds a runtime assertion preventing NaN/Inf propagation."""
...
class SoftmaxLoss(_ListwiseLoss):
    """Softmax loss with explicit runtime numerical validation."""

    def precompute(self, labels, logits, weights):
        """Precomputes stable softmax inputs."""
        labels, logits = _check_tensor_shapes([labels, logits])

        labels = _validate_finite(labels, "labels")
        logits = _validate_finite(logits, "logits")
        ...
        # Invalid entries receive a finite, deliberately low logit.
        safe_invalid_logit = tf.cast(
            _SAFE_MIN_LOGIT,
            logits.dtype,
        )
        ...
        logits = tf.compat.v1.where(
            is_valid,
            logits,
            tf.ones_like(logits) * safe_invalid_logit,
        )

        if (
            self._lambda_weight is not None
            and isinstance(self._lambda_weight, DCGLambdaWeight)
        ):
            labels = self._lambda_weight.individual_weights(
                labels,
                ranks,
            )

        if weights is not None:
            weights = _validate_finite(weights, "weights")
            labels *= weights

        labels = _validate_finite(labels, "softmax_labels")
        logits = _validate_finite(logits, "softmax_logits")

        return labels, logits

    def compute_unreduced_loss(self, labels, logits):
        """Computes unreduced softmax loss safely."""
        labels, logits = _check_tensor_shapes([labels, logits])
        ...
        nonzero_mask = tf.greater(
            tf.reshape(label_sum, [-1]),
            0.0,
        )

        padded_labels = tf.compat.v1.where(
            nonzero_mask,
            labels,
            _EPSILON * tf.ones_like(labels),
        )
        ...
        padded_label_sum = tf.reduce_sum(
            input_tensor=padded_labels,
            axis=1,
            keepdims=True,
        )

        labels_for_softmax = tf.math.divide_no_nan(
            padded_labels,
            padded_label_sum,
        )

        losses = tf.compat.v1.nn.softmax_cross_entropy_with_logits_v2(
            labels=labels_for_softmax,
            logits=logits,
        )

        losses = _validate_finite(losses, "softmax_loss")

        weights_for_softmax = tf.reshape(
            label_sum,
            [-1],
        )
...
최종 개선사항
✅ 정적 shape 검사만 의존 → 런타임 tf.debugging.assert_* 추가 → 동적 Tensor의 빈 데이터까지 방어
✅ 무차별 try/except → 입력 계약 선검증 + 원본 예외 전파 → traceback 및 장애 원인 추적성 보존
✅ log(EPSILON) 기반 수치 방어 → finite 검증 + 명시적 invalid-logit masking → NaN/Inf 전파 조기 차단
✅ 선언만 하고 사용하지 않던 _SAFE_MIN_LOGIT → 실제 invalid-entry masking에 적용 → 죽은 방어 코드 제거 및 정책 일관성 확보
✅ assert에 의존한 설정 검증 → 명시적 ValueError/TypeError 계약 검사 → 최적화 실행 환경에서도 검증 보장
✅ 알고리즘 내부 수식 무분별한 변경 → LambdaWeight·pairwise/listwise semantics 보존 → 리팩터링에 따른 모델 무결성 회귀 방지
✅ O(L²) pairwise 구조를 무리하게 변경 → 계산복잡도는 유지하고 입력·수치 방어부터 강화 → 과설계 없이 운영 안정성 우선 확보

연구용 랭킹 수식은 보존하면서 허술한 정적 검증과 가짜 수치 방어를 실제 런타임 계약·finite 검증으로 교체해, 알고리즘 무결성을 훼손하지 않고 장애 생존성을 실무 수준으로 끌어올린 구조다.        
