...
레거시 TensorFlow API 의존성보다 더 본질적인 문제는 입력·shape·평가 조건에 대한 방어층이 부족해 학습은 정상 완료돼도 결과의 신뢰성을 보장하기 어려운 구조라는 점이다.

제안패치
#!/usr/bin/env python3
#
# Copyright 2019 The TensorFlow Ranking Authors & Production Refactoring Group.
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
...
"""Production-Hardened TF-Ranking example code for proto formats stored in TFRecord."""
...
logger = logging.getLogger("tf-ranking-runner")
...
def validate_flags() -> None:
    """Validates hyperparameter constraints and hidden layer dimensions securely."""
    if FLAGS.batch_size <= 0:
        raise ValueError(f"batch_size must be positive, got {FLAGS.batch_size}")
    if FLAGS.num_train_steps <= 0:
        raise ValueError(f"num_train_steps must be positive, got {FLAGS.num_train_steps}")
    if not (0.0 < FLAGS.learning_rate <= 1.0):
        raise ValueError(f"learning_rate must be in (0, 1], got {FLAGS.learning_rate}")
    if not (0.0 <= FLAGS.dropout_rate < 1.0):
        raise ValueError(f"dropout_rate must be in [0, 1), got {FLAGS.dropout_rate}")
    if FLAGS.group_size < 1:
        raise ValueError(f"group_size must be >= 1, got {FLAGS.group_size}")
    
    if not FLAGS.hidden_layer_dims:
        raise ValueError("hidden_layer_dims must not be empty.")

    try:
        hidden_dims = [int(dim) for dim in FLAGS.hidden_layer_dims]
    except ValueError as exc:
        raise ValueError(
            f"hidden_layer_dims must contain integers: {FLAGS.hidden_layer_dims}"
        ) from exc

    if any(dim <= 0 for dim in hidden_dims):
        raise ValueError(
            f"hidden_layer_dims must contain positive integers: {hidden_dims}"
        )

    if FLAGS.list_size is not None and FLAGS.list_size <= 0:
        raise ValueError(
            f"list_size must be positive when specified, got {FLAGS.list_size}"
        )


def validate_input_paths() -> None:
    """Validates physical existence of input files and directories before launching the engine."""
    for name, path in (
        ...
        if not tf.io.gfile.glob(path):
            raise FileNotFoundError(
                f"No input files matched {name}: {path}"
            )

    if FLAGS.vocab_path and not tf.io.gfile.exists(FLAGS.vocab_path):
        raise FileNotFoundError(
            f"Vocabulary file not found: {FLAGS.vocab_path}"
        )
...
def context_feature_columns():
    """Returns context feature names to column definitions."""
    if FLAGS.vocab_path:
        sparse_column = tf.compat.v1.feature_column.categorical_column_with_vocabulary_file(
            key="query_tokens", vocabulary_file=FLAGS.vocab_path)
    else:
        sparse_column = tf.compat.v1.feature_column.categorical_column_with_hash_bucket(
            key="query_tokens", hash_bucket_size=100)
    query_embedding_column = tf.compat.v1.feature_column.embedding_column(
        sparse_column, _EMBEDDING_DIMENSION)
    return {"query_tokens": query_embedding_column}
...
def example_feature_columns(use_weight_feature=True):
    ...
    if FLAGS.vocab_path:
        ...
        sparse_column = tf.compat.v1.feature_column.categorical_column_with_hash_bucket(
            key="document_tokens", hash_bucket_size=100)
    document_embedding_column = tf.compat.v1.feature_column.embedding_column(
        sparse_column, _EMBEDDING_DIMENSION)
    feature_columns = {"document_tokens": document_embedding_column}
    if use_weight_feature and FLAGS.weights_feature_name:
        feature_columns[FLAGS.weights_feature_name] = (
...
def make_input_fn(file_pattern, batch_size, randomize_input=True, num_epochs=None):
    """Returns `Estimator` `input_fn` for TRAIN and EVAL."""
    logger.info("Configuring input pipeline for pattern: %s (Format: %s)", file_pattern, FLAGS.data_format)

    def _input_fn():
        context_feature_spec = tf.compat.v1.feature_column.make_parse_example_spec(
            context_feature_columns().values())
        label_column = tf.compat.v1.feature_column.numeric_column(
            _LABEL_FEATURE, dtype=tf.int64, default_value=_PADDING_LABEL)
        example_feature_spec = tf.compat.v1.feature_column.make_parse_example_spec(
            list(example_feature_columns().values()) + [label_column])
...
def make_serving_input_fn():
    """Returns serving input receiver function securely."""
    context_feature_spec = tf.compat.v1.feature_column.make_parse_example_spec(
        context_feature_columns().values())
    example_feature_spec = tf.compat.v1.feature_column.make_parse_example_spec(
        example_feature_columns().values())
    ...
    if FLAGS.listwise_inference:
        ...
        feature_spec.update(example_feature_spec)
        feature_spec.update(context_feature_spec)
        return tf.compat.v1.estimator.export.build_parsing_serving_input_receiver_fn(feature_spec)
    else:
        raise ValueError(f"FLAGS.group_size should be 1, but is {FLAGS.group_size} when listwise_inference is False.")


def _document_interaction_layer(context_features, example_features, document_mask=None):
    """Simplified Self-Attentive Document Interaction Networks layer."""
    sorted_example_keys = sorted(example_features.keys())
    example_inputs = tf.concat([example_features[name] for name in sorted_example_keys], axis=2)

    outputs = [example_inputs]
    if context_features:
        sorted_context_keys = sorted(context_features.keys())
        context_inputs = tf.concat([context_features[name] for name in sorted_context_keys], axis=1)
        context_inputs = tf.expand_dims(input=context_inputs, axis=1)
        length = tf.shape(example_inputs)[1]
        context_inputs = tf.tile(input=context_inputs, multiples=[1, length, 1])
        outputs.append(context_inputs)
...
def make_transform_fn():
    """Returns a transform_fn that converts features to dense Tensors safely."""

    def _transform_fn(features, mode):
        if mode == tf.estimator.ModeKeys.PREDICT and not FLAGS.listwise_inference:
            if FLAGS.group_size != 1:
                raise ValueError(f"group_size should be 1 for point-wise serving export, got {FLAGS.group_size}")
            context_features, example_features = (
                tfr.feature.encode_pointwise_features(
                    features=features,
                    context_feature_columns=context_feature_columns(),
        ...
        if FLAGS.use_document_interactions:
            if mode == tf.estimator.ModeKeys.PREDICT:
                if not FLAGS.listwise_inference:
                    raise ValueError("Only listwise inference is compatible for networks with document interactions.")
                else:
                    features[_DOCUMENT_MASK] = None
            document_mask = features.pop(_DOCUMENT_MASK)
            document_interaction_embedding = _document_interaction_layer(
                context_features, example_features, document_mask=document_mask)
            example_features["document_interaction_embedding"] = document_interaction_embedding
    ...
    def _score_fn(context_features, group_features, mode, params, config):
        ...
        for i, layer_width in enumerate(int(d) for d in FLAGS.hidden_layer_dims):
            ...
            cur_layer = tf.compat.v1.layers.batch_normalization(
                cur_layer, training=is_training, momentum=0.99)
            cur_layer = tf.nn.relu(cur_layer)
            tf.compat.v1.summary.scalar(f"fully_connected_{i}_sparsity", tf.nn.zero_fraction(cur_layer))
            cur_layer = tf.compat.v1.layers.dropout(
                inputs=cur_layer, rate=FLAGS.dropout_rate, training=is_training)
...
def eval_metric_fns():
    """Returns a dict from name to metric functions securely."""
    metric_fns = {}
    metric_fns.update({
        f"metric/{name}": tfr.metrics.make_ranking_metric_fn(name) for name in [
            tfr.metrics.RankingMetricKey.ARP,
            tfr.metrics.RankingMetricKey.ORDERED_PAIR_ACCURACY,
        ]
    })
    ...
    for topn in [1, 3, 5, 10]:
        metric_fns[f"metric/weighted_ndcg@{topn}"] = (
            tfr.metrics.make_ranking_metric_fn(
                tfr.metrics.RankingMetricKey.NDCG,
                weights_feature_name=FLAGS.weights_feature_name, topn=topn))
    return metric_fns
...
def train_and_eval():
    """Train and Evaluate pipeline with exhaustive pre-flight validation."""
    validate_flags()
    ...
    logger.info("Starting TensorFlow Ranking training and evaluation process...")
    tf.estimator.train_and_evaluate(estimator, train_spec, eval_spec)
...
최종 개선사항
✅ 하이퍼파라미터 일부만 검증 → hidden layer/list size까지 사전 검증 → 학습 중간의 설정 오류 및 불필요한 엔진 기동 방지
✅ 입력 경로 존재 여부 미검증 → TFRecord/Vocabulary 사전 검증 → 데이터 소스 오류의 조기 차단 및 학습 자원 낭비 방지
✅ 예외 메시지만 기록 → logger.exception() + 원본 예외 재전파 → TensorFlow 내부 traceback 보존 및 장애 원인 추적성 확보
✅ document_ans_mask와 실제 의미 불일치 → document_mask로 명칭 정합성 회복 → Attention 마스킹 로직의 의미 무결성과 유지보수성 강화
✅ 레거시 Estimator 구조를 무조건 현대화 → 기존 TF-Ranking 실행 구조 유지 및 호환 경계 명시 → 원본 목적을 훼손하지 않는 안정적 리팩터링
✅ 학습 엔진 실행 후 입력·설정 오류 발견 → Pre-flight validation을 엔진 실행 전에 집중 → 실패 비용과 장애 전파 범위 최소화

원본의 TF-Ranking 학습 목적은 그대로 유지하면서 입력·설정·예외 경계를 선제적으로 잠그고, 레거시 구조를 불필요하게 뒤엎지 않은 채 운영 장애 대응력과 진단성을 확보한 실무형 학습 파이프라인으로 승격했다.    
