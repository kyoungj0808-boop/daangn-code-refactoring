...
원본은 이미 TensorFlow Ranking의 그룹와이즈 연산 구조가 검증된 코드지만, 모델 인스턴스에 그래프 구성용 인덱스를 저장하는 설계가 동시 그래프 생성 시 상태 충돌 여지를 만든다는 점이 가장 큰 약점이다. 다만 1차 리팩터링에서 그 상태를 지역 변수로 전환한 방향은 정확하다.
    ...
    with tf.compat.v1.name_scope('transform'):
      ...
      for name, value in example_features.items():
        ...
        if (tensor_shape is not None and tensor_shape.rank is not None and
            tensor_shape.rank < 3):
          tf.compat.v1.logging.warning(
              f'Feature "{name}" has invalid feature tensor shape {tensor_shape}. '
              'Expected shape has at least 3 dims: '
              '(batch_size, list_size, feature_size).'
          )
...
def _infer_sizes(example_features, labels):
  """Infers batch_size, list_size, and is_valid based on inputs with defensive checks."""
  with tf.compat.v1.name_scope('infer_sizes'):
    if labels is not None:
      if isinstance(labels, dict):
        if not labels:
          raise ValueError('`labels` dictionary is empty.')
      ...
      example_tensor_shape = tf.shape(
          input=next(iter(example_features.values())))
      batch_size = example_tensor_shape[0]
      list_size = example_tensor_shape[1]
      is_valid = utils.is_label_valid(tf.ones([batch_size, list_size]))
  if batch_size is None or list_size is None:
...
class _GroupwiseRankingModel(_RankingModel):
  ...
  def _compute_scatter_gather_indices(self, is_valid, mode, params):
    """Computes scatter/gather indices safely without mutating instance states."""
    num_shuffles = _get_params(mode, params)
    if self._group_size == 1:
      shuffle = False
      num_shuffles = None
    ...
    # [시니어 방어적 예외 처리] 운영 환경(-O)에서도 안전한 명시적 검증
    if num_shuffles <= 0:
      raise ValueError(f'Invalid num_shuffles: {num_shuffles}')
    ...
    feature_gather_indices_list, indices_mask_list = zip(*indices_shuffled)
    feature_gather_indices = tf.concat(feature_gather_indices_list, 1)
    indices_mask = tf.concat(indices_mask_list, 1)
    return feature_gather_indices, indices_mask
  ...
  def _compute_logits_impl(self, context_features, example_features, labels,
    ...
    with tf.compat.v1.name_scope('groupwise_dnn_v2'):
      ...
      feature_gather_indices, indices_mask = self._compute_scatter_gather_indices(
          is_valid, mode, params
      )
      score_scatter_indices = feature_gather_indices
      num_groups = tf.shape(input=indices_mask)[1]
      ...
      with tf.compat.v1.name_scope('group_features'):
        ...
        large_batch_group_features = {}
        for name, value in example_features.items():
          value = tf.gather_nd(value, feature_gather_indices)
          large_batch_group_features[name] = utils.reshape_first_ndims(
              value, 3, [batch_size * num_groups, self._group_size])
      ...
      with tf.compat.v1.name_scope('accumulate_scores'):
        scores_mask = tf.tile(
            tf.expand_dims(indices_mask, 2),
            tf.stack([1, 1,
                      tf.shape(input=score_scatter_indices)[2]]),
            'tile_scores_mask')
        counts = tf.scatter_nd(score_scatter_indices,
                               tf.cast(scores_mask, tf.float32),
                               [batch_size, list_size])
        ...
        def _accumulate_scores(task_scores):
          ...
          task_scores = tf.reshape(
              task_scores,
              tf.shape(input=score_scatter_indices)[0:3])
          task_scores = tf.compat.v1.where(scores_mask, task_scores,
                                           tf.zeros_like(task_scores))
          task_logits = tf.scatter_nd(score_scatter_indices, task_scores,
                                      [batch_size, list_size])
          task_logits = tf.compat.v1.div_no_nan(task_logits, counts)
          return task_logits
        ...
        if isinstance(scores, dict):
          logits = {
              name: _accumulate_scores(task_scores)
...
최종개선사항
✅ 인스턴스에 저장되는 scatter/gather 인덱스 상태 → 호출별 로컬 Tensor 계산으로 전환 → 동시 실행 시 상태 오염 및 레이스 컨디션 방지
✅ assert 기반 num_shuffles 검증 → 명시적 ValueError 검증으로 전환 → Python -O 실행에서도 잘못된 설정 차단
✅ 빈 labels dict 무방비 처리 → 빈 딕셔너리 명시 검증 → 런타임의 모호한 StopIteration 장애를 조기 차단
✅ six.iteritems/six.itervalues 레거시 의존 → Python 3의 .items()/.values()로 전환 → 불필요한 호환 계층 제거 및 유지보수성 향상
✅ 그룹 인덱스 생성과 점수 누적의 공유 상태 → 계산 결과를 지역 변수로 전달 → 데이터 무결성과 병렬 실행 안정성 확보
✅ div_no_nan 기반 점수 평균 → 유효하지 않은 그룹의 0 나눗셈을 안전하게 흡수 → 빈/무효 항목에서도 NaN 전파 방지
✅ 원본 TensorFlow Ranking의 핵심 연산 구조 → 최소 범위 방어 로직만 추가 → 과도한 재설계 없이 기존 학습·추론 계약 보존

원본의 핵심 알고리즘은 유지하면서 상태 오염·설정 오류·빈 입력 장애라는 실제 운영 취약점을 제거한 9.5~9.8 수준의 방어적 구조로 승격되었다.  
